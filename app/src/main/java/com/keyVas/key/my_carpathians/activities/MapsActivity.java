...
import com.keyVas.key.my_carpathians.utils.TrackContainer;
...
public class MapsActivity extends AppCompatActivity implements OnMapReadyCallback, CommunicatorMapActivity
{
	...
	private void location() {
		iCapture = new ILocation() {
			@Override
			public void update(Location location, int typeObject) {

				if (location != null) {
					if (customProgressBarGPS.getVisibility() == View.VISIBLE) {
						enabledProgressGPS(false, typeObject);
					}
				...
				showRecLine(location, typeObject);
	...
	public void onBackPressed() {
		if (mCaptureServiceConnection != null & enabledGPS) {
	...
	private void startActionModOfflineRegion() {
		if (isOnline()) {
			if (mTypeMode) {
				mActionMode = this.startSupportActionMode(new OfflineRegionActionModeCallback(this));
	...
	private void showRegion(LatLng point) {
		...
		points.add(new LatLng(point.getLatitude() + mPerimeterMultiplierValue,
				point.getLongitude() - mPerimeterMultiplierValue));
		points.add(new LatLng(point.getLatitude() - mPerimeterMultiplierValue,
				point.getLongitude() - mPerimeterMultiplierValue));
		points.add(new LatLng(point.getLatitude() - mPerimeterMultiplierValue,
				point.getLongitude() + mPerimeterMultiplierValue));
		...
		if (selectUserRouts != null && selectUserRouts.size() > 0 && mRootPathString != null) {
			for (Rout rout :
				...
				if (rout.getUrlRoutsTrack() != null) {
					drawGeoJson(Uri.parse(rout.getUrlRoutsTrack()).getPath());
				}
			}
		}
						...
						.setPositiveButton(getString(R.string.navigate_positive_button),
								new DialogInterface.OnClickListener() {
									...
									public void onClick(DialogInterface dialog, int id) {
										...
										// Get the region bounds and zoom
										LatLngBounds bounds = (offlineRegions[mRegionSelected]
												.getDefinition())
												.getBounds();
										double regionZoom = ((OfflineTilePyramidRegionDefinition)
	...
	@Background
	public void drawGeoJson(String uri) {
		...
		try {
			...
			Toast.makeText(this, "Geo JSON Drawing failed", Toast.LENGTH_SHORT).show();
		}
		if (points.size() > 0)
			onPrepareRoteTrack(points);
	}

	@UiThread
	public void onPrepareRoteTrack(List<LatLng> points) {
		String[] mColors = {
				"#39add1", // light blue
				"#3079ab", // dark blue
				"#c25975", // mauve
	...
	public void showCreateNameDialog(final int model, String text) {
		...
		builder.setPositiveButton(getString(R.string.save_date), null);
		builder.setNegativeButton(getString(R.string.no), new DialogInterface.OnClickListener() {
			@Override
			public void onClick(DialogInterface dialogInterface, int i) {
				if ((!enabledGPS && mMarker != null) || createdTrackPosition != null) {
					removeViews(getString(R.string.cancelled_by_user));
					mapboxMap.setOnMapClickListener(null);
					mActionMode.finish();
				} else {
					Intent serviceIntent = new Intent(MapsActivity.this, LocationService.class);
					serviceIntent.putExtra(TO_SERVICE_COMMANDS, CANCEL_REC);
					MapsActivity.this.startService(serviceIntent);
					removeViews(getString(R.string.cancelled_by_user));
		...
		alert.getButton(AlertDialog.BUTTON_POSITIVE).setOnClickListener(new View.OnClickListener() {
			...
			public void onClick(View v) {
				if (nameInput.getText().toString().equals("")) {
				...
				} else if( mMarker != null || (createdTrackPosition != null
						&& createdTrackPosition.size() > 2)){
					...
					if (!enabledGPS) {
						...
						Intent serviceIntent = new Intent(MapsActivity.this,
	...
	public void saveCreatedObject(int model, String name) {
		if (model == PLACE && mMarker != null) {
			...
			mPlace.setPositionPlace(new com.keyVas.key.my_carpathians.models.Position(
					mMarker.getPosition().getLatitude(), mMarker.getPosition().getLongitude()));
			StorageSaveHelper storageSaveHelper = new StorageSaveHelper(MapsActivity.this,
					mRootPathString);
			String outcome = storageSaveHelper.savePlace(name, mPlace, false);
			...
			if (isOnline() && !enabledGPS) {
				createdTrackPosition = AltitudeFinder.extractAltitude(createdTrackPosition);
			}
			StorageSaveHelper storageSaveHelper = new StorageSaveHelper(MapsActivity.this,
					mRootPathString);
			Rout mRout = new Rout();
			mRout.setNameRout(name);
			mRout.setPositionRout(new com.keyVas.key.my_carpathians.models.
					Position(createdTrackPosition.get(0).getLatitude(),
					createdTrackPosition.get(0).getLongitude()));
			String outcome = storageSaveHelper.saveRout(name, createdTrackPosition, mRout, false);
			if (outcome.equals(FILE_EXISTS)) {
				showCreateNameDialog(0, name);
			} else {
				removeViews(outcome);
	...
	public void flashingColorAnimation() {
		if (isStartedAnimationChecker && isFlash) {
			...
			isFlash = true;
			startAnimation();
		} else if (isStartedAnimationChecker && mActionMode != null){
			MenuItem recInd = mActionMode.getMenu().findItem(R.id.recIndicator);
	...
	public void enabledProgressGPS(boolean b, int type) {
		...
		if (b) {
			...
			if (type == PLACE && mActionMode != null) {
				...
				mActionMode.setTitle(getString(R.string.action_mode_title_new_rout));
				mActionMode.setSubtitle(getString(R.string.start_rec));
			}
		}
	...
	public void showMessageAboutProblemWithGPSSignal() {
		if (customProgressBarGPS.getVisibility() == View.VISIBLE) {
			AlertDialog.Builder builder = new AlertDialog.Builder(this);
			builder.setTitle(getString(R.string.gps_signal));
			builder.setMessage(getString(R.string.gps_signal_message));
			builder.setPositiveButton(getString(R.string.wait), new DialogInterface.OnClickListener() {
				@Override
				public void onClick(DialogInterface dialog, int which) {
	...
	@Override
	public void deleteActionForGPS(int typeMode) {
		if (typeMode == ROUT){
			enabledRecAnimation(false);
		}
			finishGPSRecording();
	...
	private void finishGPSRecording() {
		Intent serviceIntent = new Intent(MapsActivity.this, LocationService.class);
		serviceIntent.putExtra(TO_SERVICE_COMMANDS, CANCEL_REC);
		MapsActivity.this.startService(serviceIntent);
		customProgressBarGPS.setVisibility(View.GONE);
	...
	private void showRecLine(Location location, int typeObject ) {
		if (typeObject == ROUT){
			createdTrackPosition = TrackContainer.getInstance().getPositionList();
			showRoutTrackOnMap(createdTrackPosition);
		}else if (typeObject == PLACE | typeObject == REGION){
			showMarkerOfPlace(new LatLng(location));
		}
	}
	private void showRecLine(LatLng latLng, int typeObject ) {
		if (typeObject == PLACE | typeObject == REGION) {
			showMarkerOfPlace(latLng);
	...
	private void showMarkerOfPlace(LatLng latLng) {
		if (mMarker == null) {
	...
	private void showRoutTrackOnMap(List<Position> createdTrackPosition) {
		if (createdTrackPosition.size() == 1) {
			IconFactory iconFactory = IconFactory.getInstance(MapsActivity.this);
			Icon iconStart = iconFactory.fromResource(R.drawable.marcer_flag_start);
			startMarker = mapboxMap.addMarker(new MarkerViewOptions().icon(iconStart).position(
					new LatLng(createdTrackPosition.get(createdTrackPosition.size() - 1).getLatitude(),
					createdTrackPosition.get(createdTrackPosition.size() - 1).getLongitude()))
					.title(getString(R.string.start)));
		} else if (createdTrackPosition.size() > 1) {
			LatLng p1 = new LatLng(createdTrackPosition.get(createdTrackPosition.size() - 1).getLatitude(),
					createdTrackPosition.get(createdTrackPosition.size() - 1).getLongitude());
			LatLng p2 = new LatLng(createdTrackPosition.get(createdTrackPosition.size() - 2).getLatitude(),
					createdTrackPosition.get(createdTrackPosition.size() - 2).getLongitude());
