...
public class StorageSaveHelper {
	...
	public String saveRout(String name, List<com.cocoahero.android.geojson.Position> positionList,
	                       Rout rout, boolean replaceExistFile) {
		...
		if (!replaceExistFile) {
				...
				if (localFile.exists()) {
					...
					if (file.exists()) {
						...
						try {
							...
							Set<String> createdByUserTrackList = new HashSet<>(
									mSharedPreferences.getStringSet(CREATED_BY_USER_ROUT_LIST,
											new HashSet<String>()));
			...
			File localFile = new File(rootPath, name);
			if (localFile.exists()) {
				File newFile  = new File(rootPath, rout.routKey());
				...
				File file = new File(rootPath2, rout.routKey());
				if (file.exists()) {
					file.delete();
				} else {
				...
				try {
					...
					if (createdByUserTrackList.add(rout.routKey())) {
	...
	public String savePlace(String name, Place place, boolean replaceExistFile){
		...
		if (!replaceExistFile) {
			...
			File file = new File(rootPath, place.placeKey());
			if (file.exists()) {
				file.delete();
			} else {
			...
			try {
				...
				if (createdByUserPlaceList.add(place.placeKey())) {
